package cn.phone.repair.module.system.api.user;
...
import cn.phone.repair.module.system.api.user.dto.AdminUserSaveReqDTO;
...
public class AdminUserApiImpl implements AdminUserApi {
    ...
    @Override
    @DataPermission(enable = false) // 忽略数据权限，避免因为过滤，导致无法查询用户。类似：https://github.com/YunaiV/ruoyi-vue-pro/issues/1051ere // 忽略租户过滤，避免因为租户ID不匹配导致无法查询用户nantIgno
    ...
    @Override
    public CommonResult<Long> createUser(AdminUserSaveReqDTO createReqDTO) {
        UserSaveReqVO user = BeanUtils.toBean(createReqDTO, UserSaveReqVO.class);
        Long userId = userService.createUser(user);
        return success(userId);
    }

    @Override
    public CommonResult<Boolean> updateUser(AdminUserSaveReqDTO updateReqDTO) {
        UserSaveReqVO user = BeanUtils.toBean(updateReqDTO, UserSaveReqVO.class);
