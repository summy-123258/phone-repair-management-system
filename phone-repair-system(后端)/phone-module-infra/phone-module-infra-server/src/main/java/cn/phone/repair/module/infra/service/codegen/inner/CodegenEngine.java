package cn.phone.repair.module.infra.service.codegen.inner;
...
import cn.phone.repair.framework.apilog.core.annotation.ApiAccessLog;
import cn.phone.repair.framework.apilog.core.enums.OperateTypeEnum;
...
import cn.phone.repair.framework.common.util.date.DateUtils;
import cn.phone.repair.framework.common.util.date.LocalDateTimeUtils;
import cn.phone.repair.framework.common.util.object.BeanUtils;
import cn.phone.repair.framework.common.util.object.ObjectUtils;
import cn.phone.repair.framework.common.util.string.StrUtils;
import cn.phone.repair.framework.excel.core.annotations.DictFormat;
import cn.phone.repair.framework.excel.core.convert.DictConvert;
...
import cn.phone.repair.module.infra.enums.codegen.CodegenVOTypeEnum;
...
public class CodegenEngine {
    ...
    /**
     * 是否为 phone-cloud 项目，用于解决 Boot 和 Cloud 的 api 模块兼容性问题
     *
     * true  - 需要有 phone-module-xxx-api 模块
    ...
    public CodegenEngine() {
        ...
        this.cloudEnable = ClassUtils.isPresent("cn.phone.repair.module.infra.framework.rpc.config.RpcConfiguration",
    ...
    private Map<String, String> getTemplates(Integer frontType) {
        ...
        if (Boolean.FALSE.equals(cloudEnable)) {
            SERVER_TEMPLATES.forEach((templatePath, filePath) -> {
                filePath = StrUtil.replace(filePath, "/phone-module-${table.moduleName}-api", "");
                filePath = StrUtil.replace(filePath, "/phone-module-${table.moduleName}-server", "");
    ...
    private static String javaModuleFilePath(String path, String module, String src) {
        return "phone-module-${table.moduleName}/" + // 顶级模块
                "phone-module-${table.moduleName}-" + module + "/" + // 子模块
                "src/" + src + "/java/${basePackage}/module/${table.moduleName}/" + path + ".java";
    }

    private static String mapperXmlFilePath() {
        return "phone-module-${table.moduleName}/" + // 顶级模块
                "phone-module-${table.moduleName}-server/" + // 子模块
    ...
    private static String vueFilePath(String path) {
        return "phone-ui-${sceneEnum.basePackage}-vue2/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3FilePath(String path) {
        return "phone-ui-${sceneEnum.basePackage}-vue3/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3UniappFilePath(String path) {
        return "phone-ui-${sceneEnum.basePackage}-uniapp/" + // 顶级目录
                "src/" + path;
    }

    private static String vue3VbenFilePath(String path) {
        return "phone-ui-${sceneEnum.basePackage}-vben/" + // 顶级目录
                "src/" + path;
    }
